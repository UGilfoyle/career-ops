// GCC-targeted scanner (repo root — used by GitHub Actions deep mode)
...
import { discoverJobsWithoutBrowser } from './dashboard-v2/scripts/lib/ddg-discovery.mjs';
import {
  titleCaseCompany,
  fetchGreenhouseJobs,
  fetchLeverJobs,
...
const userId = Number.parseInt(String(process.env.SCAN_USER_ID || process.argv[2] || 1), 10);
if (!Number.isFinite(userId)) {
  throw new Error(`Invalid SCAN_USER_ID: ${process.env.SCAN_USER_ID || process.argv[2]}`);
}
...
function buildQueries(company) {
  ...
  return [
    { name: `${company} · LinkedIn`, query: `site:linkedin.com/jobs ${quoted} ${primaryKeyword} (${locClause})`, company },
    { name: `${company} · Naukri`, query: `site:naukri.com ${quoted} ${primaryKeyword} ${locations[0] || 'Pune'}`, company },
    { name: `${company} · Greenhouse`, query: `site:boards.greenhouse.io ${quoted} ${primaryKeyword} India`, company },
  ];
}
...
function tryAdd(url, company, title, source, trustedEmployer) {
  ...
  seenUrls.add(url);
  newJobs.push({ url, canonical_url: cleanUrl, company: employer, title, source });
  stats.added++;
  return 'added';
}
      ...
      for (const q of buildQueries(company)) {
        ...
        try {
          ...
          for (const j of results) {
            ...
            if (res === 'added') process.stdout.write(`    ✓ [DDG] ${j.title.slice(0, 72)}\n`);
          }
